...
void systemsCheck() {
  digitalWrite(TST_PWR, HIGH);
  digitalWrite(CTL_PWR, HIGH);
  while (digitalRead(CTL_DAT) == 0) {
    beep(RED_PWR);
  }
  ding(RED_PWR);
  while (digitalRead(TST_DAT) == 0) {
    beep(RED_PWR);
  }
  digitalWrite(LAS_PWR, HIGH);
  ding(RED_PWR);
  while (digitalRead(CTL_DAT) == 1) {
    beep(RED_PWR);
  }
  ding(RED_PWR);
  while (digitalRead(TST_DAT) == 1) {
    beep(RED_PWR);
  }
  ding(RED_PWR);
...
void collectMeasurement() {
  ...
  digitalWrite(LAS_PWR, HIGH);
  while (digitalRead(CTL_DAT) == 1);
  while (digitalRead(TST_DAT) == 1) { measurement++; }
  digitalWrite(LAS_PWR, LOW);
...
void setupPins() {
  ...
  pinMode(CTL_PWR, OUTPUT);
  pinMode(TST_PWR, OUTPUT);
  pinMode(CTL_DAT, INPUT);
  pinMode(TST_DAT, INPUT);
